...
<head>
  ...
  <title>Departments | Windedgesoft</title>
            ...
            <div class="col-md-12">
              <div class="card card-primary">
                <div class="card-header">
                  <h3 class="card-title">Department <small>Details</small></h3>
                  <div class="card-tools">
                    <button type="button" class="btn btn-tool" data-toggle="modal" data-target="#modal-new-department" title="New Department">
                ...
                <div class="card-body">
                  <table id="tableEmployees" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?php
                      $fetchDepartments = "SELECT id, name, general_manager, status, created FROM departments ORDER BY id DESC";
                      $departmentData = mysqli_query($conn, $fetchDepartments);
                      $count = 1;

                      if ($departmentData && mysqli_num_rows($departmentData) > 0):
                        while ($department = mysqli_fetch_assoc($departmentData)):
                          ?>
                          <tr>
                            <td><?= $count++ ?></td>
                            <td><?= htmlspecialchars($department['name']) ?></td>
                            <td><?= strtoupper($department['general_manager']) ?></td>
                            <td>
                              <?php
                              $status = strtolower($department['status']);
                            ...
                            <td><?= date('d M Y, H:i', strtotime($department['created'])) ?></td>
                            <td class="project-actions text-right">
                              <a href="department-details.php?id=<?= $department['id'] ?>" class="btn btn-default btn-sm" title="View">
                                <i class="fas fa-eye"></i>
                              </a>
                              <a href="edit-department.php?id=<?= $department['id'] ?>" class="btn btn-default btn-sm" title="Edit">
        ...
        <div class="modal fade" id="modal-new-department">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <!-- form start -->
              <form action="func/department-save.php" method="post" id="newEmployeeForm">
                <div class="modal-body">
                  <div class="form-group">
                    <label>Name <span class="text-danger">*</span></label>
                    <input type="text" name="name" class="form-control" id="name" placeholder="e.g., Operations">
                  </div>
                  <div class="form-group">
                    <label>General Manager <span class="text-danger">*</span></label>
                    <select name="generalManager" class="form-control select2" id="generalManager" style="width: 100%;">
                      <option value="" selected disabled>Select General Manager</option>
                ...
                <div class="modal-footer justify-content-between">
                  ...
                  <button type="submit" name="saveDepartment" class="btn btn-primary">Save Department</button>
                </div>
              </form>
            </div>
            <!-- /.modal-content -->
    ...
    $(function () {
      ...
      $('#newEmployeeForm').validate({
        ...
        messages: {
          ...
          generalManager: {
            required: "Please select a general manager"
          },
        },
        errorElement: 'span',
